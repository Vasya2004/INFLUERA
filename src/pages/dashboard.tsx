...
import { CalendarDays, Lightbulb, Target, Users } from "lucide-react";
...
import { isIdeaInPlan, IDEA_STATUS_LABELS, migrateIdeaStatus } from "@/lib/ideas-utils";
import { PUBLICATION_STATUS_COLORS } from "@/lib/content-plan-utils";
import { motion } from "framer-motion";
import { Link } from "wouter";
import { AccentProgress, PanelAction, SectionPanel } from "@/components/app/page";
...
const ideaStatusColors: Record<IdeaStatus, string> = {
  ...
  "сценарий": "border border-violet-500/25 bg-violet-500/10 text-violet-700 dark:text-violet-300",
  "монтаж": "border border-amber-500/25 bg-amber-500/10 text-amber-700 dark:text-amber-300",
  "опубликовано": "border border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300",
};
...
const surfacePanelClass = "surface-card text-foreground";
  ...
  return (
    <div className="divide-y divide-border">
      {publications.map(pub => {
        ...
        return (
          <div key={pub.id} className="flex items-start gap-3 px-4 py-3.5 transition-colors hover:bg-muted/40 sm:items-center sm:gap-4 sm:px-5">
            <div className="flex h-10 w-10 shrink-0 flex-col items-center justify-center rounded-lg text-center leading-none">
              <div
                className={`flex h-full w-full flex-col items-center justify-center rounded-lg ${isToday ? "bg-primary text-primary-foreground" : "border border-border bg-muted/55 text-muted-foreground"}`}
              >
            ...
            <span
              className={`inline-flex max-w-[5.5rem] shrink-0 items-center truncate rounded-full border px-2 py-0.5 text-[10px] font-medium sm:max-w-none sm:px-2.5 sm:py-1 sm:text-xs ${PUBLICATION_STATUS_COLORS[pub.status as keyof typeof PUBLICATION_STATUS_COLORS] ?? "border-border bg-muted/60 text-muted-foreground"}`}
            >
              {pub.status}
            </span>
          </div>
...
export default function Dashboard() {
  ...
  const visibleIdeas = state.ideas.filter(idea =>
    migrateIdeaStatus(idea.status) !== "опубликовано"
  ...
  return (
    <div className="space-y-5 text-foreground sm:space-y-6">
      <div>
        <h1 className="text-2xl font-bold tracking-tight text-foreground sm:text-3xl">Главная</h1>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        >
          <div className={`relative min-h-[260px] overflow-hidden rounded-xl border px-4 py-4 sm:px-5 sm:py-5 ${surfacePanelClass}`}>
            <div className="pointer-events-none absolute inset-0 rounded-[inherit] bg-gradient-to-br from-primary/[0.04] to-transparent" />
            ...
            <div className="relative flex flex-col gap-4">
              <div className="flex flex-col gap-3">
                ...
                <SegmentedControl
                  ...
                  onChange={setAudiencePeriod}
                  options={audiencePeriods}
                  aria-label="Период графика аудитории"
                />
              </div>
        ...
        >
          <SectionPanel
            ...
            className={`h-full ${surfacePanelClass}`}
            action={<PanelAction href="/app/content-plan">Все</PanelAction>}
          >
          ...
          return (
            ...
            >
              <Link href={stat.href} className="block h-full">
                <div className="surface-card surface-card-interactive group relative flex h-full min-h-[120px] flex-col justify-between gap-3 rounded-xl p-3.5 sm:min-h-[130px] sm:gap-4 sm:p-4 cursor-pointer">
                  <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="flex flex-col gap-1">
                    <span className="text-2xl font-bold tracking-tight leading-none text-foreground sm:text-3xl">{stat.value}</span>
      ...
      {activeGoals.length > 0 && (
        <motion.div
          ...
          className={`relative overflow-hidden rounded-xl border ${surfacePanelClass}`}
        >
          <div className="flex items-center justify-between gap-2 border-b border-border px-4 py-3 sm:px-5 sm:py-4">
            ...
            <PanelAction href="/app/goals">Все цели</PanelAction>
          </div>
          ...
          <div className="grid divide-y divide-border sm:grid-cols-2 sm:divide-x sm:divide-y-0 lg:grid-cols-4">
            {activeGoals.slice(0, 4).map(goal => {
              ...
              return (
                <div key={goal.id} className="space-y-2.5 px-4 py-4 sm:px-5">
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-5">
        ...
        >
          <SectionPanel
            title="Идеи"
            className={`h-full ${surfacePanelClass}`}
            action={<PanelAction href="/app/ideas">Все</PanelAction>}
          >
            {dashboardIdeas.length === 0 ? (
              <div className="flex flex-col items-center px-4 py-10 text-center sm:px-5">
                <div className="mb-3 flex h-10 w-10 items-center justify-center rounded-lg border border-border bg-muted/50 text-primary">
              ...
              <div className="divide-y divide-border">
                {dashboardIdeas.map(idea => {
                  ...
                  return (
                    ...
                    >
                      <div className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-border bg-muted/50 text-primary">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-1.5 flex flex-wrap items-center gap-1.5">
                          <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[11px] font-medium ${ideaStatusColors[migrateIdeaStatus(idea.status)]}`}>
                            {IDEA_STATUS_LABELS[migrateIdeaStatus(idea.status)]}
                          </span>
        ...
        >
          <SectionPanel
            title="Платформы"
            className={`h-full ${surfacePanelClass}`}
            action={<PanelAction href="/app/platforms">Все</PanelAction>}
          >
